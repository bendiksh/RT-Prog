...
func UDP_listener(IPAddr string, chann chan string){
	sAddr, err := net.ResolveUDPAddr("udp", udp_address + ":20009")
	if err != nil {
		log.Fatalln(err)
	}
...
func UDP_IP_sender(IPAddr string){
	cAddr, err := net.ResolveUDPAddr("udp", udp_address + ":20009")
	if err != nil {
		log.Fatalln(err)
	}
